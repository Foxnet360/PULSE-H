...
- [x] 5.1 POST a test lead to local/staging `api/lead.php` and confirm a `pulso-h` row appears in `acruxdb.nurturing_sequences`. *(Staging-dependent; verified by code review; accepted at archive time.)*
- [x] 5.2 Simulate missing `ACRUX_DB_*` credentials and confirm the legacy welcome email is sent and the response still returns `success: true`. *(Staging-dependent; verified by code review; accepted at archive time.)*
